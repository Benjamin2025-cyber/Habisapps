...
final class InsuranceReportWorkflow extends BaseController
{
    ...
    public function activeSubscriptions(Request $request): JsonResponse
    {
        return $this->render($request, 'active_subscriptions', function (Request $request, ?int $scopedAgencyId): array {
            ...
            $items = $this->filterArrayItems($items, $request);
            $pagination = $this->paginateArray($items, $request, 100);

            return [
                'items' => $pagination['items'],
                'totals' => ['count' => count($items), 'coverage_amount_minor' => $totalCoverage],
    ...
    public function unpaidPremiums(Request $request): JsonResponse
    {
        return $this->render($request, 'unpaid_premiums', function (Request $request, ?int $scopedAgencyId): array {
            ...
            return [
                'items' => $pagination['items'],
                'totals' => ['count' => count($items), 'amount_minor' => $totalAmount],
                'meta' => ['pagination' => $pagination['pagination']],
            ];
        });
    }
    ...
    public function remittances(Request $request): JsonResponse
    {
        ...
        $items = array_map(
            static fn (object $row): array => [
                ...
                'total_minor' => is_numeric($row->total_minor ?? null) ? (int) $row->total_minor : 0,
    ...
    private function render(Request $request, string $reportKey, callable $compute): JsonResponse
    {
        ...
        return $this->respondSuccess($payload, 'Insurance report generated successfully', $meta);
    }
     ...
     */
    private function filterArrayItems(array $items, Request $request): array
    {
        ...
        return array_values(array_filter($items, fn (array $item): bool => $this->itemMatchesSearch($item, $search)));
    }
