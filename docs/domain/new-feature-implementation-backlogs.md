...
- Create architecture decision record before implementation.

Do not:

- Do not add `is_islamic` flag to conventional loans as implementation.
- Do not reuse interest fields.
...
- Products and contract templates require Sharia approval before use.

Do not:

- Do not allow unapproved Islamic product to originate contracts.

Tests:

- Draft product cannot be used.
- Approved product can be used.
- Approval is audited.

### G3. Asset/Project Registry

Decision:

- Islamic finance contracts must link to real asset/project records where product requires it.

Do not:

- Do not create cash-only "Islamic loan" records.
...
- Implement Murabaha as cost-plus sale receivable, not interest lending.

Do not:

- Do not calculate interest.
- Do not use conventional loan schedule tables unless ADR explicitly permits a shared receivable schedule with no interest fields.

Business rules:

1. Record asset, supplier, purchase cost, allowed costs, markup, sale price.
2. Require Sharia/product approval.
3. Contract snapshots cost and sale price.
4. Schedule collects sale receivable.
5. Accounting uses Murabaha-specific mappings.

Tests:

- Sale price equals cost plus allowed costs plus markup.
