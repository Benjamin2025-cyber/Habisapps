...
- The arrears penalty is one hybrid rule for every credit: a 5 000 XAF fixed part plus 2 % of the unpaid amount. The fixed part is a flat monthly recovery fee charged once per delinquent loan per assessment; the 2 % applies to each overdue installment. No penalty below 1 000 XAF of arrears, and the cumulative penalty on an installment never exceeds that installment's unpaid scheduled due. Nothing here is configurable per product — only the grace days are.
